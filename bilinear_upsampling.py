import torch
import nv_bilinear_upsampling_cuda as ups_cuda
...
class InterpolationFunction(torch.autograd.Function):
    ...
    def forward(ctx, input, new_h=None, new_w=None, scale_factor=None):
        ...
        outputs = ups_cuda.bilinear_forward(input.contiguous(), new_h, new_w)
    ...
    def backward(ctx, grad_o):
        outputs = ups_cuda.bilinear_backward(grad_o.contiguous(), *ctx.saved_variables)
